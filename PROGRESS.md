...
**相关 commit**：本轮改动见 `git log --grep="exp1\|baseline_hparams"`。完整结果在 `EXP1_REPORT.md` 和 `results/exp1_baseline_hparams/`。
...
**相关 commit**：本轮改动见 `git log --grep="thesis"`。完整结果在 `EXP2_REPORT.md` 和 `results/exp2_dual_graph/`。
...
4. 新增 `run_exp3_cosine_fix.py` 跑 16 组对照实验验证效果
...
**相关 commit**：本轮改动见 `git log --grep="V2\|similarity"`。V2 结果在 `EXP2_REPORT.md` 第 0 节和 `results/exp3_cosine_fix/`。
...
2. **experiments/run_exp6_ml1m.py**: 跑 6 组 ml-1m 关键对照，每组 15 轮
...
**相关 commit**：本轮改动见 `git log --grep="V3"`。完整结果在 `EXP2_REPORT.md` 第 13 节和 `results/exp4_attention, results/exp5_interest` 目录。
...
**相关 commit**：本轮改动见 `git log --grep="V4"`。22 组实验数据在 `results/exp7_tuning/`，详细分析见 `EXP2_REPORT.md` 第 14 节。
